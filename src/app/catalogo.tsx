...
export default function Catalogo({ initialProducts }: { initialProducts?: Product[] }) {
  ...
  const visibleProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      ...
      const searchable = `${product.name} ${product.category} ${product.type} ${product.size} ${product.description}`.toLocaleLowerCase("pt-BR");
  ...
  function handleCheckout(event: FormEvent<HTMLFormElement>) {
    ...
    const items = cart.map((item) => `• ${item.cartQuantity}x ${item.name} — tam. ${item.size} — ${currency.format(item.price * item.cartQuantity)}`).join("\n");
    const message = [
        ...
        <section className={styles.catalog} id="catalogo">
          ...
          <div className={styles.filterPanel}>
            ...
            <div className={styles.filterControls}>
              <label className={styles.searchBox}><span>Buscar</span><div><Search size={18} aria-hidden="true" /><input id="busca" type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Nome, cor ou medida" aria-label="Buscar no catálogo" /></div></label>
